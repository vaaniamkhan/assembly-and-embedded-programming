...
;****************************************************************
;Performs arithmetic operations in given expression:
;F = 3P + 2Q - 75
;G = 2P - 4Q + 63
;Result = F + G
; where F, G, P, and Q are variables.
;Name:  Vaania Khan
;Date:  2026-01-29
;Class:  CMPE-250
;Section:  Section 01, Tuesday 2:00 PM
;---------------------------------------------------------------
...
;>>>>> begin main program code <<<<<
			...
			ADDS R0, R0, R1		;R0 <- 45 + 6 = 51
			
			RSBS R6, R2, #0		;R6 <- -13
			ASRS R6, R6, #2		;R6 <- -13 / 4 = -4
			
			ADDS R0, R0, R6		;R0 <- -4 + 51 = 47
			
			LSLS R7, R3, #1		;R7 <- 7 * 2
			ADDS R3, R7, #7		;R3 <- (7 * 2) + 7 = 21
			RSBS R3, R7, #0		;R7 <-  -21
			
			ADDS R0, R0, R7	;R0 <- 47 + (-21) = 26
			
			RSBS R3, R4, #0		;R3 <- -65
			ADDS R3, R3, R5		;R5 <- -65 + 33 = -32
			
			ADDS R0, R0, R5		;R0 <- -32 + 26 = -6
